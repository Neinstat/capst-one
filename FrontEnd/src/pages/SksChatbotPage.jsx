    ...
    <div className="p-6 max-w-5xl mx-auto space-y-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-300 max-w-xl font-medium leading-relaxed">
              Chatbot cerdas untuk estimasi konversi SKS jalur prestasi dan magang MBKM. Pilih jalur, lalu ceritakan kegiatanmu.
            </p>
          </div>

          {step === 'chat' && (
            <button
              onClick={() => { setStep('jalur'); setJalur(null); setSubJalur(null); setMessages([]) }}
      ...
      {step === 'jalur' && (
        <div className="bg-white/80 backdrop-blur-md rounded-3xl border border-gray-100 p-7 max-w-lg shadow-sm">
          <h2 className="text-xs font-extrabold text-gray-400 uppercase tracking-widest mb-5">Pilih Jalur Konversi</h2>
          <div className="space-y-3">
            {Object.values(SKS_JALUR).map((j) => {
              ...
              return (
                <button
                  key={j.id}
                  onClick={() => { setJalur(j); setSubJalur(null) }}
                ...
                >
                  <div className={`w-2.5 h-2.5 rounded-full mt-1.5 flex-shrink-0 transition-colors ${isSelected ? 'bg-rose-500' : 'bg-gray-300'}`} />
                  <div className="flex-1">
                    <p className="text-sm font-extrabold text-gray-900">{j.label}</p>
                    <p className="text-xs font-semibold text-gray-500 mt-0.5 leading-relaxed">{j.description}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/80 backdrop-blur-md rounded-3xl border border-gray-100 p-6 shadow-sm sticky top-6">
              <h3 className="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-4">Tabel Konversi Resmi DTI</h3>
              <div className="space-y-0">
                {CONVERSION_TABLE.map(({ type, sks }) => (
                  <div key={type} className="flex items-start justify-between py-3 border-b border-gray-50 last:border-0 gap-3">
                    <p className="text-xs text-gray-600 font-semibold leading-snug">{type}</p>
                    <span className="text-[11px] font-extrabold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-lg border border-rose-100/60 flex-shrink-0">{sks}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Konteks Aktif</p>
          ...
          {/* Chat Interface */}
          <div className="lg:col-span-2 flex flex-col bg-white/80 backdrop-blur-md rounded-3xl border border-gray-100 shadow-sm overflow-hidden" style={{ minHeight: 520 }}>
            {/* Chat Header */}
            <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-extrabold text-gray-900">MBKM AI Agent</p>
                <p className="text-[10px] font-semibold text-gray-400">{jalur?.label} · {subJalur}</p>
              </div>
